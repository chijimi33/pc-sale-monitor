...
同一巡回の店舗には同じ `--run-id` を指定します。`--seconds` は実行環境の時間制限に合わせた中断時刻で、Web呼出数の上限ではありません。キュー・商品観測・履歴への受け渡しを商品ごとに原子的に保存し、次回は未完了位置から再開します。現在比較には今回の `observed_run_id` を持つ価格だけを使います。取得障害は在庫切れに変換しません。同じ巡回で失敗したページは直後の関連候補で再取得しません。一時的な通信失敗・混雑は最低5分とサーバー指定の待機期限を過ぎれば、後続の別タスクから同じページを再取得できます。403・404等はその巡回内では再取得せず、失敗タスクは元の登録日時を保って次回へ繰り越します。キャッシュは正本として使用しません。
...
ツクモの比較検索は、HTTP 404でも検索語・ページタイトル・専用の「該当商品なし」表示が一致する場合だけ正常な空結果として記録します。`comparison_no_results` は今回の該当件数です。通常の404や取得障害は未処理に残し、検索の空結果から既存商品の在庫・価格を変更しません。

セール一覧・セール商品の再確認を優先したうえで、比較処理は新着・値下げ・在庫復活の優先度、元の依頼日時の順に進めます。同じ優先度の未処理を毎回の新しい価格確認が追い越さないようにします。比較検索から一覧・商品へ進む際も元の `created_at` を引き継ぎ、待ち時間をリセットしません。登録済みの未処理も新しい変化で優先度が上がれば更新します。
...
- ドスパラは対象商品の見出しにあるメーカー区切りから完全な商品表記を保持する。型番末尾・色・容量・限定モデルの表記を削らない。明示されたメーカー型番があればそれを優先し、区切りが不明な商品名や関連商品の見出しから型番を推測しない。
- 数値差不足、比較店不足、履歴不足を別々に残す。履歴範囲外を含めた「年間最安」とは呼ばない。
- arkの対象商品価格欄と構造化データの期限が食い違う場合は、双方の証拠を保存し `expiry_conflict_review_needed` として通知を保留する。関連商品の期限は比較せず、年のない表示から期限を推測しない。
